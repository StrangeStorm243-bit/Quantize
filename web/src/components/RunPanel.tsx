      ...
      <ul className="rpanel__list">
        {rows.map((row) => (
          ...
          >
            ...
            {/* The served return through the shared display formatter (D-27); the number's own
                element carries the bare verbatim value (the shared verbatim-title shape). */}
            <span className="rpanel__row-meta">
              {row.mode} · {row.ok ? 'ok' : 'failed'} · ret{' '}
              <span {...verbatimTitle(row.total_return)}>{fmtValue(row.total_return)}</span>
            </span>
